...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Prince') }}</h4>
            </div>
        </div>
    ...
    <div class="panel panel-flat">
        ...
        @endif
            ...
            <form action="{{ isset($find) ? url('admin/edit_prince_comments/'.$find->id) : url('admin/add_prince_comments') }}" method="post" class="form-horizontal" enctype='multipart/form-data' style="margin:20px;">
                @csrf
                ...
                {{--  <div class="form-group">
                    <label class="control-label col-sm-3" for="prince">{{ trans('app.Prince') }}</label>
                    <div class="col-md-9">
                        <input id="prince" type="text" class="form-control{{ $errors->has('prince') ? ' is-invalid' : '' }}" name="prince_id" value="{{ isset($find->prince_id) ? $find->prince_id : ''}}"  placeholder="{{ trans('app.Prince') }}">

                        @if ($errors->has('prince'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('prince') }}</strong>
                            </span>
                        @endif
                    </div>
                </div>  --}}

                <input id="prince" type="hidden" class="form-control{{ $errors->has('prince') ? ' is-invalid' : '' }}" name="prince_id" value="{{ isset($find->prince_id) ? $find->prince_id : ''}}"  placeholder="{{ trans('app.Prince') }}">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        ...
                        @if(isset($find))
                            <img src="{{ url('Admin/images/prince/'.$find->image) }}" name="df_image" style="width:80px;height:80px;"/>
