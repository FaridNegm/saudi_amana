...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Sub_Sites') }}</h4>
            </div>
        </div>
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Sub_Sites') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title col-sm-6 pull-left">{{ trans('app.Sub_Sites') }}</h5>
            <a href="{{ url('admin/add_sub_sites') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
        ...
        <table class="table table-bordered table-hover datatable-highlight">
            ...
            <tbody>
                @foreach ($all as $sub_sites)
                    <tr>
                        <td>{{ $sub_sites->id}}</td>
                        <td>{{ $sub_sites->title}}</td>
                        <td>
                            <a href="{!! $sub_sites->url !!}" target="_blank">{!! $sub_sites->url !!}</a>

                        </td>
                        <td>
                            <img src="{{ url('Admin/images/sub_sites/'.$sub_sites->image) }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
                            @if ($sub_sites->status == 'active')
