...
@section('title')
    Fekra| {{ trans('app.Prince Comments') }}
@endsection
...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Prince Comments') }}</h4>
            </div>
        </div>
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Prince Comments') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <a href="{{ url('admin/add_prince_comments') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
        ...
        <table class="table table-bordered table-hover datatable-highlight">
            ...
            <tbody>
                @foreach ($all as $prince)
                    <tr>
                        <td>{{ $prince->id}}</td>
                        <td>{{ $prince->name}}</td>
                        <td>{{ $prince->email}}</td>
                        <td>{!! $prince->message !!}</td>
                        <td>{!! $prince->prince['title'] !!}</td>
                        <td>
                            <img src="{{ url('Admin/images/prince/user.png') }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
                            @if ($prince->status == 'active')
