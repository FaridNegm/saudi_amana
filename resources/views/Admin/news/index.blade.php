...
@section('content')
    ...
    <div class="page-header page-header-default">
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.All News') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <a href="{{ url('admin/add_news') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
            ...
            <tbody>
                @foreach ($all as $news)
                    <tr>
                        ...
                        <td>
                            <img src="{{ url('Admin/images/news/'.$news->image) }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
                            @if ($news->slider == '1')
                                <span class="label label-primary label-rounded">{{ trans('app.Active') }}</span>
                            @else
                                <span class="label label-default label-rounded">{{ trans('app.Not Active') }}</span>
                            @endif
                        </td>
                        
                        <td>
                            @if ($news->add_to_news_nav == '1')
                                <span class="label label-primary label-rounded">{{ trans('app.Active') }}</span>
                            @else
                                <span class="label label-default label-rounded">{{ trans('app.Not Active') }}</span>
                            @endif
