...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Gate') }}</h4>
            </div>
        </div>
    ...
    <div class="panel panel-flat">
        ...
        @endif
            ...
            <form action="{{ isset($find) ? url('admin/edit_gate_comments/'.$find->id) : url('admin/add_gate_comments') }}" method="post" class="form-horizontal" enctype='multipart/form-data' style="margin:20px;">
                @csrf
                ...
                {{--  <div class="form-group">
                    <label class="control-label col-sm-3" for="gate">{{ trans('app.Gate') }}</label>
                    <div class="col-md-9">
                        <input id="gate" type="text" class="form-control{{ $errors->has('gate') ? ' is-invalid' : '' }}" name="gate_id" value="{{ isset($find->gate_id) ? $find->gate_id : ''}}"  placeholder="{{ trans('app.Gate') }}">
                ...
                <input id="gate" type="hidden" class="form-control{{ $errors->has('gate') ? ' is-invalid' : '' }}" name="gate_id" value="{{ isset($find->gate_id) ? $find->gate_id : ''}}"  placeholder="{{ trans('app.Gate') }}">
