...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Gate Comments') }}</h4>
            </div>
        </div>
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Gate Comments') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <a href="{{ url('admin/add_gate_comments') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
            ...
            <tbody>
                @foreach ($all as $gate)
                    <tr>
                        ...
                        <td>{{ $gate->email}}</td>
                        <td>{!! $gate->message !!}</td>
                        <td>{!! $gate->gate['title'] !!}</td>
                        <td>
                            <img src="{{ url('Admin/images/gate/user.png') }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
