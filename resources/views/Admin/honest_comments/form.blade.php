...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Honest') }}</h4>
            </div>
        </div>
    ...
    <div class="panel panel-flat">
        ...
        @endif
            ...
            <form action="{{ isset($find) ? url('admin/edit_honest_comments/'.$find->id) : url('admin/add_honest_comments') }}" method="post" class="form-horizontal" enctype='multipart/form-data' style="margin:20px;">
                @csrf
                ...
                {{--  <div class="form-group">
                    <label class="control-label col-sm-3" for="honest">{{ trans('app.Honest') }}</label>
                    <div class="col-md-9">
                        <input id="honest" type="text" class="form-control{{ $errors->has('honest') ? ' is-invalid' : '' }}" name="honest_id" value="{{ isset($find->honest_id) ? $find->honest_id : ''}}"  placeholder="{{ trans('app.Honest') }}">

                        @if ($errors->has('honest'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('honest') }}</strong>
                            </span>
                        @endif
                    </div>
                </div>  --}}

                <input id="honest" type="hidden" class="form-control{{ $errors->has('honest') ? ' is-invalid' : '' }}" name="honest_id" value="{{ isset($find->honest_id) ? $find->honest_id : ''}}"  placeholder="{{ trans('app.Honest') }}">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        ...
                        @if(isset($find))
                            <img src="{{ url('Admin/images/honest/'.$find->image) }}" name="df_image" style="width:80px;height:80px;"/>
