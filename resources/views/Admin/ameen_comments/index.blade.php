...
@section('title')
    Fekra| {{ trans('app.Ameen Comments') }}
@endsection
...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Ameen Comments') }}</h4>
            </div>
        </div>
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Ameen Comments') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <a href="{{ url('admin/add_ameen_comments') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
        ...
        <table class="table table-bordered table-hover datatable-highlight">
            ...
            <tbody>
                @foreach ($all as $ameen)
                    <tr>
                        <td>{{ $ameen->id}}</td>
                        <td>{{ $ameen->name}}</td>
                        <td>{{ $ameen->email}}</td>
                        <td>{!! $ameen->message !!}</td>
                        <td>{!! $ameen->ameen['title'] !!}</td>
                        <td>
                            <img src="{{ url('Admin/images/ameen/user.png') }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
                            @if ($ameen->status == 'active')
