...
@section('title')
    Fekra| {{ trans('app.Honest Comments') }}
@endsection
...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Honest Comments') }}</h4>
            </div>
        </div>
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Honest Comments') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title col-sm-6 pull-left">{{ trans('app.All Comments') }}</h5>
            <a href="{{ url('admin/add_honest_comments') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
        ...
        <table class="table table-bordered table-hover datatable-highlight">
            ...
            <tbody>
                @foreach ($all as $honest)
                    <tr>
                        <td>{{ $honest->id}}</td>
                        <td>{{ $honest->name}}</td>
                        <td>{{ $honest->email}}</td>
                        <td>{!! $honest->message !!}</td>
                        <td>{!! $honest->honest['title'] !!}</td>
                        <td>
                            <img src="{{ url('Admin/images/honest/user.png') }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
                            @if ($honest->status == 'active')
