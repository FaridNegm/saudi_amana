...
@section('content')
    ...
    <div class="page-header page-header-default">
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Calends') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title col-sm-6 pull-left">{{ trans('app.All Calends') }}</h5>
            <a href="{{ url('admin/add_calends') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
        ...
        <table class="table table-bordered table-hover datatable-highlight">
            ...
            <tbody>
                @foreach ($all as $calends)
                    <tr>
                        <td>{{ $calends->id}}</td>
                        <td>{{ $calends->title}}</td>
                        <td>
                            <a href="{{ url('Admin/files/calends/'.$calends->file) }}" download>{{ $calends->file }}</a>
                        </td>
                        <td>
                            @if ($calends->status == 'active')
