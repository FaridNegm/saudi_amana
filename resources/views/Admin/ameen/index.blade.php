...
@section('content')
    ...
    <div class="page-header page-header-default">
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Ameens') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title col-sm-6 pull-left">{{ trans('app.All Ameens') }}</h5>
            <a href="{{ url('admin/add_ameen') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
            ...
            <tbody>
                @foreach ($all as $ameen)
                    <tr>
                        <td>{{ $ameen->id}}</td>
                        <td>{{ $ameen->title}}</td>
                        <td>{!! $ameen->description !!}</td>
                        <td>
                            <img src="{{ url('Admin/images/ameen/'.$ameen->image) }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
                            @foreach ($ameen->ameen_comments as $message)
                                <ul>
