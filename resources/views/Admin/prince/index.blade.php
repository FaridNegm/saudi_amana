...
@section('content')
    ...
    <div class="page-header page-header-default">
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Prince') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title col-sm-6 pull-left">{{ trans('app.All Prince') }}</h5>
            <a href="{{ url('admin/add_prince') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
            ...
            <tbody>
                @foreach ($all as $prince)
                    <tr>
                        <td>{{ $prince->id}}</td>
                        <td>{{ $prince->title}}</td>
                        <td>{!! $prince->description !!}</td>
                        <td>
                            <img src="{{ url('Admin/images/prince/'.$prince->image) }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
                            @foreach ($prince->prince_comments as $message)
                                <ul>
