...
@section('content')
    ...
    <div class="page-header page-header-default">
        <div class="page-header-content">
            <div class="page-title">
                <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{ trans('app.home') }}</span> - {{ trans('app.Ameens') }}</h4>
            </div>
        </div>
    ...
    <div class="panel panel-flat">
        ...
        @endif
            ...
            <form action="{{ isset($find) ? url('admin/edit_ameen_comments/'.$find->id) : url('admin/add_ameen_comments') }}" method="post" class="form-horizontal" enctype='multipart/form-data' style="margin:20px;">
                @csrf
                ...
                {{--  <div class="form-group">
                    <label class="control-label col-sm-3" for="ameen">{{ trans('app.Ameens') }}</label>
                    <div class="col-md-9">
                        <input id="ameen" type="text" class="form-control{{ $errors->has('ameen') ? ' is-invalid' : '' }}" name="ameen_id" value="{{ isset($find->ameen_id) ? $find->ameen_id : ''}}"  placeholder="{{ trans('app.Ameens') }}">

                        @if ($errors->has('ameen'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('ameen') }}</strong>
                            </span>
                        @endif
                    </div>
                </div>  --}}

                <input id="ameen" type="hidden" class="form-control{{ $errors->has('ameen') ? ' is-invalid' : '' }}" name="ameen_id" value="{{ isset($find->ameen_id) ? $find->ameen_id : ''}}"  placeholder="{{ trans('app.Ameens') }}">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        ...
                        @if(isset($find))
                            <img src="{{ url('Admin/images/ameen/'.$find->image) }}" name="df_image" style="width:80px;height:80px;"/>
