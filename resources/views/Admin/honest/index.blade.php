...
@section('content')
    ...
    <div class="page-header page-header-default">
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Honest') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title col-sm-6 pull-left">{{ trans('app.All Honest') }}</h5>
            <a href="{{ url('admin/add_honest') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
            ...
            <tbody>
                @foreach ($all as $honest)
                    <tr>
                        <td>{{ $honest->id}}</td>
                        <td>{{ $honest->title}}</td>
                        <td>{!! $honest->description !!}</td>
                        <td>
                            <img src="{{ url('Admin/images/honest/'.$honest->image) }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
                            @foreach ($honest->honest_comments as $message)
                                <ul>
